...
public class DIS_2120 {
	...
	public static void   main(String []args) throws SQLException, JSONException{
		Log.initialization("Sprint33.1/DIS-2120");	
		//List<String> detail =  ExcelUtils.getuserDetails();
		try {
			//startDate=EpochTime.getEpochTime(""+startDate+"");
			String excelPath="./testData/Sprint33.1/DIS-2120.xlsx";
			workbook = new XSSFWorkbook(excelPath);
			sheet= workbook.getSheet("Authenticate");
			...
			clientID= formatter.formatCellValue(sheet.getRow(i).getCell(7));
			...
			bookID3 = fetchBookList_without_pagination.then().extract().path("bookList.book.id[0]");			
			System.out.println("bookID3: "+bookID3);				
			isbn = fetchBookList_without_pagination.then().extract().path("bookList.book.isbn[0]");
			...
			Validation.responseKeyValidation_key(downloadBook1,"timestamp");



			Response downloadBook = DownloadBook.downloadBook(userToken,"ds9465",deviceT[i4],bookID1,"online");
			Log.info("bookID1 : "+bookID1);
			...
			downloadBook = DownloadBook.downloadBook(userToken,"ds9465","HTML5",bookID1,"offline");
			Log.info("bookID1 : "+bookID1);
			Validation.responseCodeValidation1(downloadBook, HttpStatus.SC_OK);
			Validation.responseTimeValidation(downloadBook);
			Validation.responseKeyValidation_key(downloadBook,"fileSize");
